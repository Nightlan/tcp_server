...
/// @file Defines a single-writer/single-reader FIFO lock-free queue.

#ifndef EPOLL_SPSC_QUEUE_H__
#define EPOLL_SPSC_QUEUE_H__
...
class BatchQueue {
 public:
  ...
  inline ~BatchQueue () {
    ...
    Chunk *sc = spare_chunk_.exchange(nullptr, std::memory_order_relaxed);
    free (sc);
  }
  ...
  inline int Init() {
    ...
    if (nullptr == begin_chunk_) return EPOLL_FAIL;
    begin_pos_ = 0;
    back_chunk_ = nullptr;
    back_pos_ = 0;
  ...
  inline void Pop () {
    if (++ begin_pos_ == N) {
      ...
      //  use 'o' as the spare.
      Chunk* cs = spare_chunk_.exchange(o, std::memory_order_relaxed);
      free(cs);
    }
  }
 ...
 private:
  ...
  //  Disable copying of BatchQueue
  DISALLOW_CONSTRUCTORS(BatchQueue);
};
...
class SpscQueue {
 public:
  ...
  //  Following function (write) deliberately copies uninitialized data
  //  when used with zmq_msg. Initializing the VSM body for
  //  non-VSM messages won't be good for performance.

#ifdef KMQ_HAVE_OPENVMS
#pragma message save
...
#pragma message restore
  ...
  inline bool Flush() {
    ...
    if (!contention_.compare_exchange_strong(w_item_, f_item_, 
      ...
      contention_.store(f_item_, std::memory_order_relaxed);
      w_item_ = f_item_;
      return false;
    }
 ...
 protected:
  ...
  //  atomic operations.
  std::atomic<T*> contention_;

  //  Disable copying of SpscQueue
  DISALLOW_CONSTRUCTORS(SpscQueue);
};

#endif // EPOLL_SPSC_QUEUE_H__